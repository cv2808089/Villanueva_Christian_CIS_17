...
Stats *stat(const Array *array){
     ...
     if(stats->modFreq > 1)
    {
        for(int i = 0; i < array->size-1; i++)
        {
            ...
            if(maxh2 == stats->modFreq)
            {
                stats->mode->data[h] = array->data[i];
                h++;
            }
            maxh2 = 1;
        }
    ...
    if((array->size)%2 == 0)
    {
         int m1 = (array->size/2)-1;
        int m2 = array->size/2;
        stats->median = (array->data[m1] + array->data[m2])/2.0;
    }
    else
    {
        mc = (array->size+1)/2;
        stats->median = array->data[mc-1];
    }
