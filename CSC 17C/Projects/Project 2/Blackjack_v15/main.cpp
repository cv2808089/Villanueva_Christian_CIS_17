...
//outputs highest value played
string highCard(Hand, player, int, int);
//outputs lowest value played
string lowCard(Hand, player, int, int);
void heapSort(int arr[], int);
stack<Card> formDeck();
unsigned int JSHash(const std::string&);
int hSearch(list<string> arr[], string);
...
int main()
{
    ...
    if(winc == win)
    {
        AVLTree<Card> winningHand;
        for(int i = 0; i < pcp; i++)
            winningHand.root = winningHand.insert(winningHand.root, p1.getCard(i));
        winningHand.inorder(winningHand.root);
    }
    if(winc == lose)
    {
    ...
    cout << "The highest card value played this game was: " << highCard(dealer, p1, dcp, pcp) << endl;
    cout << "The lowest card value played this game was: " << lowCard(dealer, p1, dcp, pcp) << endl;
    //outputs end message
    cout << endl;
    ...
    cout << endl << endl;
    int hsh;
    string arr2[] = {"this" , "is", "a", "test", "blackjack"};
    list<string> arr[5];
    for(int i = 0; i < 5; i++)
    {
        hsh = JSHash(arr2[i]);
        arr[hsh].push_back(arr2[i]);
    }
    
    string val = "blackjack";
    if(hSearch(arr, val) == -1)
        cout << "Something's wrong" << endl;
    
    
    cout << "Now its time to go home, finding the shortest distance home using the 5 airports available" << endl;
