...
class AVLTree{
    public:
        ...
        void search(BNTnode<T>*, T);   //search tree and count nodes traversed
...
template <class T>
void AVLTree<T>::search(BNTnode<T> *root, T value)
{
    int nt = 0;
    while(root != NULL)
    {
        if(root->data == value)
        {
            cout << "Nodes traversed: " << nt << endl;
            return;
        }
        nt++;
        root = root->data < value ? root->right : root->left;
    }
    cout << "Item not found" << endl;
    cout << "Nodes traversed(tree): " << nt << endl;
}
#endif	/* AVLTREE_H */