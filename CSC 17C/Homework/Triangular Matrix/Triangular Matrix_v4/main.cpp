...
int main(int argc, char** argv) {
    ...
    cin>>m1.size;
    m1.indx=fillIdx(m1.size);      //Index used for Database Sort
    m1.col=fillAry(m1.size);       //Dynamic 1-D array,#Columns in each row
    m1.data=fillAry(m1.col,m1.size);//Dynamic triangular array
    
    //Output Located Here
    cout<<"The Column Array Size"<<endl;
    prntAry(m1.col,m1.size);
    cout<<"The Triangular Array Sorted"<<endl;
    mrkSort(m1.col,m1.indx,m1.size);//Sort the columns with an index
    prntAry(m1.data,m1.col,m1.indx,m1.size);//Display the Matrix with Columns sorted

    //Deallocate the array
    destroy(m1.data,m1.col,m1.indx,m1.size);
...
int **fillAry(int *col,int rows){
    ...
    for(int i=0;i<rows;i++){
        int tcol = mrkRand()%rows+1;
        array[i]=new int[tcol];
